# AGENTS.md - GotifyNotification 项目工作指引

## 项目概述

GotifyNotification 是一款 Windows 桌面应用，连接 Gotify 服务器并通过 Windows 原生通知展示消息。
...
- **数据存储**：SQLite 在 `%APPDATA%/GotifyNotification/gotify_messages.db`，设置 JSON 在同目录子文件夹
...
- 数据库：用 DB Browser for SQLite 打开 `%APPDATA%/GotifyNotification/gotify_messages.db`
